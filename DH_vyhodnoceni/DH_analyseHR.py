...
class AnalyseHR:
    ...
    def analyze_epochs(self):
        ...
        i=1
        ws.cell(row=1, column=i).value = "Číslo epochy"
        i+=1
        ws.cell(row=1, column=i).value = "Čas začátku epochy"
        i+=1
        ws.cell(row=1, column=i).value = "HR"
        i+=1
        ws.cell(row=1, column=i).value = "RR min"
        i+=1
        ws.cell(row=1, column=i).value = "RR max"
        i+=1
        ws.cell(row=1, column=i).value = "RMSSD"
        i+=1
        ws.cell(row=1, column=i).value = "SDNN"
        i+=1
        ws.cell(row=1, column=i).value = "Počet nádechů"


        for epoch_num in range(pocet_epoch):
            i=1
            ws.cell(row=epoch_num+2, column=i).value = epoch_num+1
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["time"][epoch_num]
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["HR"][epoch_num]
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["RR-min"][epoch_num]
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["RR-max"][epoch_num]
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["RMSSD"][epoch_num]
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["SDNN"][epoch_num]
            i+=1
            ws.cell(row=epoch_num+2, column=i).value = self.epoch_stats["RESP"][epoch_num]

        wb.save(filename=f"{self.filename}_epochy.xlsx")
        """
